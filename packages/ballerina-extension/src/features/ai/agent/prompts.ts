...
- Define required configurables for the query. Use only string, int, decimal, boolean and array types in configurable variables. Never a langlib-qualified numeric subtype (e.g. \`int:Signed32\`, \`int:Unsigned16\`) even if a connector's client method parameter uses one; those are not supported as configurable types and fail at runtime. Widen to the base type (\`int\`) and cast when calling the client. Never assign hardcoded default values to configurables.
