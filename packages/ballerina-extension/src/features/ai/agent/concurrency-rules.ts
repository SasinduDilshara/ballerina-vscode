...
- In a lock that touches an isolated root (an \`isolated\` variable or \`self\` of an isolated object), a MUTABLE value leaving the lock (returned or assigned to an outer variable) must not alias the protected state: copy it with \`value.clone()\` (or \`value.cloneReadOnly()\` when an immutable result is acceptable). Immutable (\`readonly\`) values and single isolated objects (a client, a caller) may cross the boundary freely. Do NOT make a value \`readonly\` merely to bring it into a lock when it will later be stored as mutable state — see the cloning rule below. An ordinary lock over non-isolated state has no such transfer restrictions — do not add gratuitous clones there.
