...
public class SymbolProcessor {
    ...
    /**
     * Applies member-field annotation attachments to the {@link TypeDef} fields (matched by name),
     * for both record fields and object fields.
     */
    private static void applyFieldAnnotations(TypeDefinitionSymbol typeDefSymbol,
        ...
        TypeSymbol rawType = CommonUtils.getRawType(typeDefSymbol.typeDescriptor());
        Map<String, ? extends Annotatable> fieldSymbolsByName;
        if (rawType instanceof RecordTypeSymbol recordType) {
            fieldSymbolsByName = mapFieldSymbolsByName(recordType.fieldDescriptors());
        } else if (rawType instanceof ObjectTypeSymbol objectType) {
            fieldSymbolsByName = mapFieldSymbolsByName(objectType.fieldDescriptors());
        } else {
            return;
        }

        for (Field field : fields) {
            Annotatable fieldSymbol = fieldSymbolsByName.get(field.getName());
            if (fieldSymbol == null) {
                continue;
            }
    ...
    /**
     * Re-keys the given field descriptors on the field's own name, falling back to the descriptor
     * key. Accepts any annotatable field symbol, so it serves both {@link RecordFieldSymbol} and
     * {@link io.ballerina.compiler.api.symbols.ObjectFieldSymbol} maps.
     */
    private static <T extends Symbol & Annotatable> Map<String, T> mapFieldSymbolsByName(
            Map<String, ? extends T> fieldDescriptors) {
        Map<String, T> fieldSymbolsByName = new HashMap<>();
        for (Map.Entry<String, ? extends T> entry : fieldDescriptors.entrySet()) {
            T fieldSymbol = entry.getValue();
            fieldSymbolsByName.put(fieldSymbol.getName().orElse(entry.getKey()), fieldSymbol);
        }
        return fieldSymbolsByName;
    }
}
