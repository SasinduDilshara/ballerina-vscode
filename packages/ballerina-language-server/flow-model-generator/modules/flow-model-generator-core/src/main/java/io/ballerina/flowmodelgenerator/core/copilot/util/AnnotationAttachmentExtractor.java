 ...
 *
 * <p>Compiler-internal {@code ballerina/lang.*} annotations (e.g. {@code @deprecated},
 * {@code @strand}, {@code @typeParam}) are skipped as noise &mdash; {@code @deprecated} is already
 * surfaced separately via the {@code isDeprecated} flags. Only the user-facing ones, currently
 * {@code @display}, are retained (see {@link #USER_FACING_LANG_ANNOTATIONS}).</p>
 *
 * @since 1.7.0
 */
public final class AnnotationAttachmentExtractor {
    ...
    /**
     * The {@code ballerina/lang.*} annotations that are meaningful to write in user code. Everything
     * else in those modules is compiler machinery ({@code strand}, {@code typeParam},
     * {@code isolatedParam}, ...) and is noise in generated code; {@code deprecated} is already
     * surfaced through the {@code isDeprecated} flags. An allow-list rather than a deny-list, so a
     * lang annotation added by a future compiler is excluded by default instead of leaking.
     */
    private static final Set<String> USER_FACING_LANG_ANNOTATIONS = Set.of("display");
    ...
    public static List<AnnotationAttachment> extract(Annotatable annotatable, String currentOrg,
        ...
        for (AnnotationAttachmentSymbol attachmentSymbol : annotatable.annotAttachments()) {
            ...
            attachmentSymbol.attachmentValue()
                    ...
                    .filter(AnnotationAttachmentExtractor::isAttachableValue)
    ...
    private static boolean isInternalAnnotation(AnnotationSymbol annotationSymbol, String name) {
        ...
        return isLangModule && !USER_FACING_LANG_ANNOTATIONS.contains(name);
    }

    /**
     * Whether the rendered value may legally follow an annotation tag.
     *
     * <p>An annotation attachment's value must be a mapping constructor, so only a rendered mapping is
     * attachable. An annotation declared without a record/map constraint — {@code @cloud:Expose},
     * declared {@code public const annotation Expose on parameter;} — reports the constant
     * {@code true} as its value; emitting that would produce {@code @cloud:Expose true}, which does
     * not parse. Such an annotation is attached bare.</p>
     */
    private static boolean isAttachableValue(String rendered) {
        return rendered != null && rendered.startsWith("{");
    }
    ...
    /** Escapes the characters that would otherwise break out of a Ballerina string literal. */
